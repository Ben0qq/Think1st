...
export const Input:React.FC<Props & InputHTMLAttributes<HTMLInputElement>> = ({label,name,formData,setValue,errors,...props}) =>{
    ...
    {label}
    <input {...props} onChange={(e)=>{setValue({...formData, [name]:e.target.value});}} className={classNames(" rounded-lg px-4 h-12 font-medium focus:border-2 focus:border-[#761BE4] focus:bg-[#FAF9FA] ",{"bg-white border border-[#CBB6E5]":!errors[name],"border-2 border-[#ED4545] bg-[#FEECEC]":errors[name]})}/>
